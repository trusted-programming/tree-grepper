...
fn main() {
    ...
    //asm
    let asm_dir: PathBuf = ["vendor", "tree-sitter-asm", "src"].iter().collect();

    println!("cargo:rerun-if-changed=vendor/tree-sitter-asm/src/parser.c");
    cc::Build::new()
        .include(&asm_dir)
        .warnings(false)
        .file(asm_dir.join("parser.c"))
        .compile("tree-sitter-asm");
